...
local leading_term, curr_index, polys;
...
local F, F_t, num_iter, curr_gb, multipliers, G, repeat;
print("Step 1");
...
while true do
  ...
  print("Step 3 & 4");
  curr_gb, multipliers := Basis(F_t, order_2, output=extended);
  print("Matrix of multipliers M': ", multipliers);
  print("H (= M' F_t): ", curr_gb);
  
  print("Step 5");
  G := simplify(Multiply(convert(multipliers, Matrix), Transpose(convert(F, Matrix))));
  print("G (= M' * F): ", G);
